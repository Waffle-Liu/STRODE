...
class FullNN_bigphi(nn.Module):
    def __init__(self, h_dim, time_dim=64, n_full_layers=2):
        super(FullNN_bigphi, self).__init__()
...
class FullNN_smallphi(nn.Module):
    def __init__(self, h_dim, time_dim=64, n_full_layers=2):
        super(FullNN_smallphi, self).__init__()
        self.time_dim = time_dim
        self.n_full_layers = n_full_layers
...
class FullNN(nn.Module):
    def __init__(self, time_dim=64, n_full_layers=2):
        super(FullNN, self).__init__()
        self.time_dim = time_dim
        self.n_full_layers = n_full_layers

        self.linear_time = NonnegativeLinear(1, time_dim)
        self.dense_p = nn.Linear(3*64*64, time_dim)
        self.linear_layers = nn.ModuleList(
            [NonnegativeLinear(time_dim, time_dim) for _ in range(n_full_layers - 1)])
        self.final_layer = NonnegativeLinear(time_dim, 1)
    ...
    def mlp(self, input_x, input_rnn):
        ...
        for i in range(self.n_full_layers-1):
            hidden = self.linear_layers[i](hidden)
            hidden = torch.tanh(hidden)

        hidden = self.final_layer(hidden)
        hidden = F.softplus(hidden)

        return hidden + input_x
...
class DiffeqSolver_Parallel(nn.Module):
    def __init__(self, input_dim, ode_func, method, latents,
                 odeint_rtol=1e-4, odeint_atol=1e-5, device=torch.device("cpu")):
        super(DiffeqSolver_Parallel, self).__init__()
        self.ode_method = method
        self.latents = latents
        self.device = device
        self.ode_func = ode_func
...
class DiffeqSolver_Parallel_new(nn.Module):
    def __init__(self, input_dim, ode_func, method, latents,
                 odeint_rtol=1e-4, odeint_atol=1e-5, device=torch.device("cpu")):
        super(DiffeqSolver_Parallel_new, self).__init__()
        self.ode_method = method
        self.latents = latents
        self.device = device
        self.ode_func = ode_func
    ...
    def forward(self, y0, delta_t):
        # y0 (10, 1, 32)
        # t_seq (10, 1, 1)
        k = 10
        delta_t = delta_t/k
        ti = torch.zeros_like(delta_t).to(self.device)
        yi = y0
        for i in range(k):
            yi = self.perform_one_step(self.ode_func, yi, ti, delta_t)
            ti = ti + delta_t
...
class DiffeqSolver_FP(nn.Module):
    def __init__(self, input_dim, ode_func, method, latents,
                 odeint_rtol=1e-4, odeint_atol=1e-5, device=torch.device("cpu")):
        super(DiffeqSolver_FP, self).__init__()
        self.ode_method = method
        self.latents = latents
        self.device = device
        self.ode_func = ode_func
    ...
    def forward(self, y0, t_seq):
        ...
        # t_seq (10, 1, 1)
        k = opt.ode1_step
        delta_t = t_seq[:, :, 1].unsqueeze(2) - t_seq[:, :, 0].unsqueeze(2)

        ti = t_seq[:, :, 0].unsqueeze(2)
        yi = y0
        solutions = []
        ...
        for i in range(delta_t.shape[1]):
            dt = delta_t[:, i:i+1, :]/k
            for i in range(k):
                yi = self.perform_one_step(self.ode_func, yi, ti, dt)
                ti = ti + dt

            solutions.append(yi)
        solutions = torch.cat(solutions, dim=1)
...
class DiffeqSolver_KL(nn.Module):
    ...
    def forward(self, b_phi_zt, b_phi_zt_deriv, s_phi_zt, s_phi_zt_deriv):
        ...
        K = s_phi_zt + torch.log(-1.*b_phi_zt_deriv+1e-6) - \
            torch.log(s_phi_zt_deriv+1e-6)
...
class STRODE(nn.Module):
    def __init__(self, in_dim, h_dim, kernel_size, num_layers, time_dim1=16, n_full_layers1=2, time_dim2=16, n_full_layers2=2, device=torch.device('cuda')):
        ...
        self.n_layers2 = 1
        ode_in_dim = h_dim
        ode_latent_dim = h_dim
        self.device = device

        self.encode_xt1 = nn.Sequential(
            nn.Linear(in_dim, h_dim),
            nn.ReLU(),
            nn.Linear(h_dim, h_dim)
        )
        self.fullnn1 = FullNN_bigphi(h_dim, time_dim1, n_full_layers1)

        self.encode_xt2 = nn.Sequential(
            nn.Linear(in_dim, h_dim),
            nn.ReLU(),
            nn.Linear(h_dim, h_dim)
        )

        self.fullnn2 = FullNN_smallphi(h_dim, time_dim2, n_full_layers2)
        ...
        self.ode_solver = DiffeqSolver_Parallel(input_dim=h_dim, ode_func=self.odefunc, method='euler', latents=10, device=device)
    ...
    def forward(self, x, mode='train'):
        ...
        enc_x1 = self.encode_xt1(x)
        input_x1 = torch.zeros((b, 1), requires_grad=True).to(self.device)
        integral1_list = []
        derivative1_list = []
        for i in range(l):
            integral1, derivative1 = self.fullnn1(input_x1, enc_x1[:, i, :], mode)
        ...
        enc_x2 = self.encode_xt2(x)
        input_x2 = torch.zeros((b, 1), requires_grad=True).to(self.device)
        integral2_list = []
        derivative2_list = []
        for i in range(l):
            integral2, derivative2 = self.fullnn2(input_x2, enc_x2[:, i, :], mode)
        ...
        for j in range(integral1_list.shape[1]):
            ...
            zt_inv = self.ode_solver(enc_x1[:, j, :], t_seq)
            zt_inv_list.append(zt_inv)
        qzt_inv = torch.stack(zt_inv_list, dim=1)

        qzt_inv = qzt_inv.reshape(b, l, -1)
        qzt_inv = self.decode(qzt_inv)
        x_next = torch.cat((x[:, 1:, :], x[:, -1:, :]), dim=1)
        mse = torch.mean(F.mse_loss(qzt_inv, x_next))

        if mode == 'train': 
            kl2 = self.kl_pt_qt(integral1_list, derivative1_list, integral2_list, derivative2_list)
        else:
            kl2 = 0
        ...
        return loss, integral1_list.squeeze(-1), mse, kl2

    def kl_pt_qt(self, integral1, derivative1, integral2, derivative2):
        loss = torch.zeros((opt.train_batch_size, integral1.shape[1])).to(self.device)
        for i in range(integral1.shape[1]):
